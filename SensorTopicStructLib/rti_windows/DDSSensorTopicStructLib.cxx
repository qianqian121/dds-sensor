...
/* ========================================================================= */
const char *SensorWapperTopicStructTYPENAME = "SensorWapperTopicStruct";

DDS_TypeCode* SensorWapperTopicStruct_get_typecode()
{
    static RTIBool is_initialized = RTI_FALSE;

    static DDS_TypeCode SensorWapperTopicStruct_g_tc_sensor_1_string = DDS_INITIALIZE_STRING_TYPECODE((255));
    static DDS_TypeCode_Member SensorWapperTopicStruct_g_tc_members[3]=
    {

        {
            (char *)"attribute_2",/* Member name */
            {
                0,/* Representation ID */          
                DDS_BOOLEAN_FALSE,/* Is a pointer? */
                -1, /* Bitfield bits */
    ...
    static DDS_TypeCode SensorWapperTopicStruct_g_tc =
    {{
            DDS_TK_STRUCT,/* Kind */
            DDS_BOOLEAN_FALSE, /* Ignored */
            -1, /*Ignored*/
            (char *)"SensorWapperTopicStruct", /* Name */
            NULL, /* Ignored */      
            0, /* Ignored */
            0, /* Ignored */
            NULL, /* Ignored */
            3, /* Number of members */
            SensorWapperTopicStruct_g_tc_members, /* Members */
            DDS_VM_NONE  /* Ignored */         
        }}; /* Type code for SensorWapperTopicStruct*/

    if (is_initialized) {
        return &SensorWapperTopicStruct_g_tc;
    }

    SensorWapperTopicStruct_g_tc_members[0]._representation._typeCode = (RTICdrTypeCode *)&DDS_g_tc_octet;

    SensorWapperTopicStruct_g_tc_members[1]._representation._typeCode = (RTICdrTypeCode *)SensorTopicStruct_get_typecode();

    SensorWapperTopicStruct_g_tc_members[2]._representation._typeCode = (RTICdrTypeCode *)&SensorWapperTopicStruct_g_tc_sensor_1_string;
...
RTIBool SensorWapperTopicStruct_initialize(
    ...
    return SensorWapperTopicStruct_initialize_ex(sample,RTI_TRUE,RTI_TRUE);
}

RTIBool SensorWapperTopicStruct_initialize_ex(
    SensorWapperTopicStruct* sample,RTIBool allocatePointers, RTIBool allocateMemory)
{
    ...
    return SensorWapperTopicStruct_initialize_w_params(
        sample,&allocParams);

}

RTIBool SensorWapperTopicStruct_initialize_w_params(
    SensorWapperTopicStruct* sample, const struct DDS_TypeAllocationParams_t * allocParams)
{

    if (allocParams) {} /* To avoid warnings */

    if (!RTICdrType_initOctet(&sample->attribute_2)) {
        return RTI_FALSE;
    }     

    if (!SensorTopicStruct_initialize_w_params(&sample->sensor_0,
    allocParams)) {
        return RTI_FALSE;
    }

    if (allocParams->allocate_memory){
        sample->sensor_1= DDS_String_alloc ((255));
    ...
    SensorWapperTopicStruct* sample)
{
    ...
    SensorWapperTopicStruct_finalize_ex(sample,RTI_TRUE);
}

void SensorWapperTopicStruct_finalize_ex(
    SensorWapperTopicStruct* sample,RTIBool deletePointers)
{
    struct DDS_TypeDeallocationParams_t deallocParams =
    DDS_TYPE_DEALLOCATION_PARAMS_DEFAULT;
    ...
    SensorWapperTopicStruct_finalize_w_params(
        sample,&deallocParams);
}

void SensorWapperTopicStruct_finalize_w_params(
    SensorWapperTopicStruct* sample,const struct DDS_TypeDeallocationParams_t * deallocParams)
{
    ...
    SensorTopicStruct_finalize_w_params(&sample->sensor_0,deallocParams);

    if (sample->sensor_1 != NULL) {
        DDS_String_free(sample->sensor_1);
        sample->sensor_1=NULL;
    ...
    SensorWapperTopicStruct* sample, RTIBool deletePointers)
{
    ...
    SensorTopicStruct_finalize_optional_members(&sample->sensor_0, deallocParams->delete_pointers);
}
    ...
    const SensorWapperTopicStruct* src)
{
    ...
    if (!RTICdrType_copyOctet (
        &dst->attribute_2, &src->attribute_2)) { 
        return RTI_FALSE;
    }
    if (!SensorTopicStruct_copy(
        &dst->sensor_0, &src->sensor_0)) {
        return RTI_FALSE;
    } 
    if (!RTICdrType_copyStringEx (
        &dst->sensor_1, src->sensor_1, 
        (255) + 1, RTI_FALSE)){
        return RTI_FALSE;
    }
...
* Defines:  TSeq, T
*
* Configure and implement 'SensorWapperTopicStruct' sequence class.
*/
#define T SensorWapperTopicStruct
#define TSeq SensorWapperTopicStructSeq
#define T_initialize_w_params SensorWapperTopicStruct_initialize_w_params
#define T_finalize_w_params   SensorWapperTopicStruct_finalize_w_params
#define T_copy       SensorWapperTopicStruct_copy
